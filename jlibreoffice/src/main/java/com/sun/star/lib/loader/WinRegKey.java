 ...
 *  $Revision: 1.3 $
 *
 *  last change: $Author: 205327 $ $Date: 2012/01/02 21:24:26 $
 *
...
final class WinRegKey {
    ...
    private static native boolean winreg_RegOpenCurrentConfig(long[] hkresult );
    ...
    private static native boolean winreg_RegOpenKeyEx( long parent, String name, long[] hkresult );
    private static native boolean winreg_RegCloseKey( long hkey );    
    private static native boolean winreg_RegQueryValueEx( long hkey, String value, long[] type, byte[] data, long[] size );
    ...
    static {        
        try {
            ...
            if ( is != null ) {                
            	...
            	// Verifica se a biblioteca esta salva no temp
    ...
    private byte[] getValue( String valueName ) throws WinRegKeyException {
        ...
        boolean bRet = false;
        long[] hroot = {0};
        if (  keyName.getRootKeyName().equals( "HKEY_CLASSES_ROOT" ) ) {
            bRet = winreg_RegOpenClassesRoot( hroot );
        } else if ( keyName.getRootKeyName().equals( "HKEY_CURRENT_CONFIG" ) ) {
            bRet = winreg_RegOpenCurrentConfig( hroot );
        } else if ( keyName.getRootKeyName().equals(WinRegKeyName.HKEY_CURRENT_USER) ) {
            bRet = winreg_RegOpenCurrentUser( hroot );
        } else if ( keyName.getRootKeyName().equals(WinRegKeyName.HKEY_LOCAL_MACHINE) ) {
            bRet = winreg_RegOpenLocalMachine( hroot );
        } else if ( keyName.getRootKeyName().equals( "HKEY_USERS" ) ) {
        ...
        if ( !winreg_RegOpenKeyEx( hroot[0],keyName.getSubKeyName(), hkey ) ) {
